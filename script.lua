-- ============================================================
-- PHANTOM DUELS - SCRIPT COMPLETO
-- ============================================================
-- Identidade: PHANTOM DUELS
...
local function autoSaveConfig()
    ...
    task.delay(0.5, function()
        ...
        pcall(function() _writefile("PhantomDuelsConfig.json", HttpService:JSONEncode(cfg)) end)
        saveDebounce = false
    end)
end
...
-- INSTANT RESET
-- ============================================================
local resetPos = Vector3.new(0.076, 99997.35, -0.517)
local activeResetClone = nil

local function cleanupReset()
    if activeResetClone then
        activeResetClone:Destroy()
        activeResetClone = nil
    end
    workspace.CurrentCamera.CameraSubject = LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
    workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
    pcall(function()
        local hGui = LP.PlayerGui:FindFirstChild("Health")
        if hGui then hGui.Enabled = true end
    end)
end
...
local function doInstantReset()
    ...
    pcall(function()
        ...
        if head then
            local died = head:FindFirstChild("Died")
            if died then died.Volume = 0 end
        end
    end)
    ...
    task.spawn(function()
        while activeResetClone and LP.Character == char do
...
-- ============================================================
-- MOBILE PANEL - PHANTOM DUELS THEME
-- ============================================================
local function createMobilePanel()
    local panel = Instance.new("ScreenGui")
    panel.Name = "PhantomDuelsButtons"
    ...
    local DARK_PURPLE = Color3.fromRGB(100, 40, 200)
    ...
    local function makeSeparateBtn(name, text, defaultPos, saveKey, callback, isToggle)
        ...
        local function setActive(state)
            active = state
            if active then
                container.BackgroundColor3 = PURPLE
                btn.TextColor3 = WHITE
        ...
        btn.MouseButton1Down:Connect(function()
            if not isToggle then 
                container.BackgroundColor3 = DARK_PURPLE
                btn.TextColor3 = WHITE
            end
        end)
    ...
    local dropBRSetActive = makeSeparateBtn("BtnDropBR", "DROP\nBR",
        UDim2.new(1, COL1_X, BASE_Y, ROW_Y[1]), "PhantomBtn_dropbr.txt",
    ...
    local autoLeftSetActive = makeSeparateBtn("BtnAutoLeft", "AUTO\nLEFT",
        UDim2.new(1, COL2_X, BASE_Y, ROW_Y[1]), "PhantomBtn_autoleft.txt",
        function(setActive)
    ...
    local autoBatSetActive = makeSeparateBtn("BtnAutoBat", "BAT\nAIMBOT",
        UDim2.new(1, COL1_X, BASE_Y, ROW_Y[2]), "PhantomBtn_autobat.txt",
        function(setActive)
    ...
    local autoRightSetActive = makeSeparateBtn("BtnAutoRight", "AUTO\nRIGHT",
        UDim2.new(1, COL2_X, BASE_Y, ROW_Y[2]), "PhantomBtn_autoright.txt",
        function(setActive)
    ...
    local tpDownSetActive = makeSeparateBtn("BtnTpDown", "TP\nDOWN",
        UDim2.new(1, COL1_X, BASE_Y, ROW_Y[3]), "PhantomBtn_tpdown.txt",
        function(setActive, currentActive)
            tpToGround()
    ...
    local carrySpeedSetActive = makeSeparateBtn("BtnCarrySpd", "CARRY\nSPD",
        UDim2.new(1, COL2_X, BASE_Y, ROW_Y[3]), "PhantomBtn_carryspd.txt",
        function(setActive)
            toggleSpeedType()
            setActive(State.speedType == "carry")
        end, true)

    local laggerCarrySetActive = makeSeparateBtn("BtnLaggerCarry", "LAGGER\nCARRY",
        UDim2.new(1, COL1_X, BASE_Y, ROW_Y[4]), "PhantomBtn_laggercarry.txt",
        function(setActive)
            toggleLaggerCarry()
            setActive(State.laggerCarryActive)
        end, true)

    local laggerSetActive = makeSeparateBtn("BtnLaggerMode", "LAGGER\nMODE",
        UDim2.new(1, COL2_X, BASE_Y, ROW_Y[4]), "PhantomBtn_laggermode.txt",
        function(setActive)
            toggleLagger()
            setActive(State.laggerActive)
        end, true)

    local dropAimbotSetActive = makeSeparateBtn("BtnDropAimbot", "DROP\nAIMBOT",
        UDim2.new(1, COL1_X, BASE_Y, ROW_Y[4] + BTN_H + GAP), "PhantomBtn_dropaimbot.txt",
        function(setActive, currentActive)
            runDropBrainrot()
            if setMenuDropBR then setMenuDropBR(true) end
            ...
            task.delay(0.60, function()
                ...
                enableAutoBat()
                if setAutoBat then setAutoBat(true) end
                if MobileButtons.Buttons and MobileButtons.Buttons.autoBat then
                    MobileButtons.Buttons.autoBat(true)
                end
            end)
            setActive(true)
            task.delay(0.8, function() setActive(false) end)
        end, false)
    ...
    MobileButtons.Buttons = {
        ...
        dropAimbot = dropAimbotSetActive,
    }
...
-- ============================================================
-- GUI INTERFACE - PHANTOM DUELS THEME
-- ============================================================
-- Cores PHANTOM DUELS
local PURPLE = Color3.fromRGB(160, 80, 255)
local DARK_PURPLE = Color3.fromRGB(100, 40, 200)
...
local PANEL_BG = Color3.fromRGB(14, 14, 20)
local ROW_BG = Color3.fromRGB(22, 22, 32)
local ROW_HOV = Color3.fromRGB(35, 30, 50)
local BORDER = Color3.fromRGB(80, 40, 180)
local BORDER2 = Color3.fromRGB(120, 60, 220)
local ACCENT = Color3.fromRGB(160, 80, 255)
local ACCENT2 = Color3.fromRGB(180, 120, 255)
local DIM = Color3.fromRGB(100, 80, 140)
local ON_BG = Color3.fromRGB(80, 40, 180)
local OFF_BG = Color3.fromRGB(20, 20, 30)
local KEY_BG = Color3.fromRGB(0, 0, 0)
...
for _, name in pairs({"JispiHubGUI","MonkeyHubGUI","PhantomDuelsGUI"}) do
...
-- ============================================================
-- AUTO STEAL INDICATOR - PHANTOM DUELS THEME
-- ============================================================
local stealProgressBar = Instance.new("Frame", gui)
stealProgressBar.Size = UDim2.new(0, 280, 0, 64)
stealProgressBar.Position = UDim2.new(0.5, -140, 1, -80)
stealProgressBar.BackgroundColor3 = BLACK
...
local spStroke = Instance.new("UIStroke", stealProgressBar)
spStroke.Color = PURPLE
...
barTrack.Size = UDim2.new(1, -28, 0, 14)
barTrack.Position = UDim2.new(0, 14, 0, 40)
barTrack.BackgroundColor3 = Color3.fromRGB(20, 20, 30)
...
local function saveStealBarPosition()
    ...
    local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
    pcall(function() _writefile("PhantomStealBarPos.txt", str) end)
end

local function loadStealBarPosition()
    local savedPos = nil
    pcall(function() savedPos = _readfile("PhantomStealBarPos.txt") end)
    if savedPos and savedPos ~= "" then
        local parts = {}
        for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
        if #parts >= 4 then
...
-- ============================================================
-- MAIN GUI FRAME - PHANTOM DUELS THEME
-- ============================================================
...
mainGrad.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(14, 12, 24)),
...
local function saveMainPosition()
    ...
    local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
    pcall(function() _writefile("PhantomDuelsGUIPos.txt", str) end)
end
...
local function saveMiniPosition()
    if miniBtn then
        ...
        local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
        pcall(function() _writefile("PhantomMiniPos.txt", str) end)
    end
end

local function loadMainPosition()
    local savedPos = nil
    pcall(function() savedPos = _readfile("PhantomDuelsGUIPos.txt") end)
    if savedPos and savedPos ~= "" then
        local parts = {}
        for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
        if #parts >= 4 then
...
local function loadMiniPosition()
    local savedPos = nil
    pcall(function() savedPos = _readfile("PhantomMiniPos.txt") end)
...
headerGrad.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.3, Color3.fromRGB(140, 70, 240)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(160, 80, 255)),
    ColorSequenceKeypoint.new(0.7, Color3.fromRGB(120, 60, 220)),
...
headerDiv.BackgroundColor3 = PURPLE
...
titleLbl.Position = UDim2.new(0, 14, 0.5, -11)
...
closeBtn.BackgroundColor3 = Color3.fromRGB(30, 28, 40)
...
closeBtnStroke.Color = PURPLE
closeBtnStroke.Thickness = 1
closeBtnStroke.Transparency = 0.5
closeBtnRef = closeBtn

closeBtn.MouseEnter:Connect(function()
    TweenService:Create(closeBtn, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(180, 40, 60)}):Play()
    TweenService:Create(closeBtnStroke, TweenInfo.new(0.12), {Color = Color3.fromRGB(220, 60, 80)}):Play()
    TweenService:Create(closeBtn, TweenInfo.new(0.12), {TextColor3 = WHITE}):Play()
end)
closeBtn.MouseLeave:Connect(function()
    TweenService:Create(closeBtn, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(30, 28, 40)}):Play()
    TweenService:Create(closeBtnStroke, TweenInfo.new(0.12), {Color = PURPLE}):Play()
    TweenService:Create(closeBtn, TweenInfo.new(0.12), {TextColor3 = PURPLE}):Play()
end)
...
miniBtn.Name = "PhantomMiniButton"
...
miniBtn.Text = "PHANTOM DUELS"
miniBtn.TextColor3 = PURPLE
...
miniBtn.MouseEnter:Connect(function()
    TweenService:Create(miniBtn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(20, 18, 30)}):Play()
end)
miniBtn.MouseLeave:Connect(function()
    TweenService:Create(miniBtn, TweenInfo.new(0.1), {BackgroundColor3 = BLACK}):Play()
end)
...
local function makeInputRow(label, default, onChange)
    ...
    box.Focused:Connect(function()
        TweenService:Create(bs, TweenInfo.new(0.15), {Color = PURPLE, Transparency = 0}):Play()
        box.BackgroundColor3 = Color3.fromRGB(8, 8, 12)
    end)
    
    box.FocusLost:Connect(function(enterPressed)
        TweenService:Create(bs, TweenInfo.new(0.15), {Color = PURPLE, Transparency = 0.5}):Play()
        box.BackgroundColor3 = BLACK
...
local function makeActionBtn(label, onClick)
    ...
    btn.MouseButton1Click:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.08), {BackgroundColor3 = PURPLE}):Play()
        task.delay(0.16, function() 
            TweenService:Create(btn, TweenInfo.new(0.12), {BackgroundColor3 = PANEL_BG}):Play() 
        end)
        if onClick then pcall(onClick) end
    end)
    btn.MouseEnter:Connect(function() 
        TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = ROW_HOV}):Play() 
    end)
    btn.MouseLeave:Connect(function() 
        TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = PANEL_BG}):Play() 
    end)
    return btn
end
...
local function makeToggleRow(label, defaultKey, defaultOn, onToggle, onKeyChanged, keybindStateKey)
    ...
    if defaultKey and not keybindStateKey then
        ...
        local function kStop(key)
            ...
            TweenService:Create(ks, TweenInfo.new(0.12), {Color = PURPLE, Transparency = 0.5}):Play()
            keyBtn.TextColor3 = PURPLE
            if key then 
        ...
        keyBtn.MouseButton1Click:Connect(function()
            ...
            TweenService:Create(ks, TweenInfo.new(0.12), {Color = PURPLE, Transparency = 0}):Play()
    ...
    if keybindStateKey then
        ...
        newKeyBtn.BackgroundColor3 = KEY_BG
        newKeyBtn.BorderSizePixel = 0
        newKeyBtn.Text = keyDisplayName
        newKeyBtn.TextColor3 = PURPLE
        newKeyBtn.Font = Enum.Font.FredokaOne
        ...
        local function nkStop(key)
            ...
            TweenService:Create(nks, TweenInfo.new(0.12), {Color = PURPLE, Transparency = 0.5}):Play()
            newKeyBtn.TextColor3 = PURPLE
            if key then
        ...
        newKeyBtn.MouseButton1Click:Connect(function()
            ...
            nkListening = true
            newKeyBtn.Text = "..."
            newKeyBtn.TextColor3 = WHITE
            TweenService:Create(nks, TweenInfo.new(0.12), {Color = PURPLE, Transparency = 0}):Play()
    ...
    local function setV(on)
        ...
        TweenService:Create(pillBg, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
            BackgroundColor3 = on and ON_BG or OFF_BG
        }):Play()
        TweenService:Create(pStroke, TweenInfo.new(0.2), {
            Color = on and PURPLE or Color3.fromRGB(40, 40, 60)
        }):Play()
...
setInstaGrab = makeToggleRow("Auto Grab (Progression)", nil, true, function(on)
    AutoSteal.Enabled = on
    if on then startAutoSteal() else stopAutoSteal() end
end)
setInfJump = makeToggleRow("Infinite Jump", nil, true, function(on) State.infJumpEnabled = on end)
...
holdBtn.BackgroundColor3 = OFF_BG
holdBtn.BorderSizePixel = 0
holdBtn.Text = "Hold"
holdBtn.TextColor3 = DIM
holdBtn.Font = Enum.Font.FredokaOne
holdBtn.TextSize = 12
holdBtn.AutoButtonColor = false
holdBtn.ZIndex = 5
Instance.new("UICorner", holdBtn).CornerRadius = UDim.new(0, 8)
Instance.new("UIStroke", holdBtn).Color = Color3.fromRGB(40, 40, 60)
...
local function updateInfJumpModeUI()
    if State.infJumpMode == "manual" then
        ...
        manuelBtn.BackgroundColor3 = OFF_BG
        manuelBtn.TextColor3 = DIM
        holdBtn.BackgroundColor3 = ON_BG
...
setAutoTpDown = makeToggleRow("Auto Tp Down", nil, true, function(on)
...
setAntiRag = makeToggleRow("Anti Ragdoll", nil, true, function(on)
    State.antiRagdollEnabled = on
    if on then startAntiRagdoll() else stopAntiRagdoll() end
end)
setFps = makeToggleRow("FPS Boost", nil, true, function(on)
    State.fpsBoostEnabled = on
    if on then pcall(applyFPSBoost) end
end)
setMedusaCounter = makeToggleRow("Medusa Counter", nil, true, function(on)
...
local setDropAimbot
setDropAimbot = makeToggleRow("Drop Aimbot", nil, false, function(on)
    if on then
        runDropBrainrot()
        if setMenuDropBR then setMenuDropBR(true) end
        task.delay(0.5, function() if setMenuDropBR then setMenuDropBR(false) end end)
...
autoSaveLbl.TextColor3 = PURPLE
autoSaveLbl.Font = Enum.Font.FredokaOne
...
footerLbl.Text = "PHANTOM DUELS  ·  v2.0"
footerLbl.TextColor3 = PURPLE
footerLbl.Font = Enum.Font.FredokaOne
...
applyFPSBoost = function()
    ...
    workspace.DescendantAdded:Connect(function(v)
        if State.fpsBoostEnabled then task.spawn(processObj, v) end
    end)
end
...
local function loadConfig()
    local hasFile = false
    pcall(function() hasFile = _isfile("PhantomDuelsConfig.json") end)
    if not hasFile then return end
    local ok, cfg = pcall(function() return HttpService:JSONDecode(_readfile("PhantomDuelsConfig.json")) end)
    ...
    if type(cfg.autoBatToggled) == "boolean" then 
        State.autoBatToggled = cfg.autoBatToggled
        if State.autoBatToggled then
            enableAutoBat()
        else
            disableAutoBat()
...
local function setupChar(char)
    ...
    if head then
        ...
        speedLbl.Font = Enum.Font.FredokaOne
...
task.spawn(function()
    task.wait(1)
    if AutoSteal.Enabled then pcall(startAutoSteal) end
    if State.fpsBoostEnabled then pcall(applyFPSBoost) end
    if State.medusaCounterEnabled and LP.Character then pcall(setupMedusaCounter, LP.Character) end
    if State.antiRagdollEnabled and not Conns.antiRag then pcall(startAntiRagdoll) end
    if State.autoBatToggled then pcall(enableAutoBat) end
end)

-- ============================================================
-- CARREGAR SCRIPT TP BAT EXTERNO
-- ============================================================
task.spawn(function()
    pcall(function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/SCRIPTBR1/TP-BAT/main/script.lua"))()
    end)
end)
